...
def communityAnalysis(csv_file_name):
    ...
    for comunidade_id in df['comunidade'].unique():
        ...
        dados_positivos.append(contagem_gostou)
        dados_negativos.append(contagem_nao_gostou)
        ...
        dados_sentimento_medio.append(sentimento_medio)
        ### print("Sentimento médio da comunidade:", sentimento_medio)

    largura_barra = 0.35  # Largura das barras
    posicoes = np.arange(2)
    plt.bar(posicoes, dados_positivos, largura_barra, label='Tendência neutra-positiva')
    plt.bar(posicoes + largura_barra, dados_negativos, largura_barra, label='Tendência neutra-negativa')   
    
    # Configura o gráfico de análise sentimental (por quantidade)
    plt.xlabel('Comunidades')
    plt.ylabel('Quantidade de comentários')
    
    plt.rcParams['figure.dpi'] = 300
    titulo = csv_file_name.split('_')[0]
    plt.title('Análise sentimental por comunidades - ' + titulo)
    plt.xticks(posicoes + largura_barra / 2, ['#1', '#2'])
    plt.legend()
    
    plt.show()
    
    plt.barh(['#1', '#2'], dados_sentimento_medio)

    # Configura o gráfico de análise sentimental (por média)
    plt.xlabel('Valor')
    plt.ylabel('Comunidades')
    plt.title('Sentimento médio por comunidade - ' + titulo)
